...
class NPCLAD:
    ...
    def initialize(self):
        ...
        # Draw the prior posterior $ \\boldsymbol{\pi}_{n|n-1} $ from the Dirichlet distribution $ Dir \left( c_{s}; \\boldsymbol{\\alpha}_{0} \right) $
        pis_0 = np.ramdom.dirichlet(self.alphas)
        ...
        # Initialize other beliefs
        self.bs_n_1_n = np.zeros(self.cs, order='C', dtype=bool)
        self.bz_n_1_n = np.zeros(self.cz, order='C', dtype=bool)

        # TODO: initialize the observation properly (create sparse vector and add it to WNN layer 0)
        self.zn = np.zeros(self.dz, order='C', dtype=bool)

        # Initialize layers
        self.wnn_layer0 = VGRAMNode(pattern_length=self.dz, min_mem_size=1, max_mem_size=2 ^ 14, min_dist=2, max_dist=8)
        self.wnn_layer1 = VGRAMArray(output_dims=(self.cs, 1), pattern_length=dz + cs, min_mem_size=1, max_mem_size=2 ^ 14,
                                     min_dist=2, max_dist=8)
        self.wnn_layer2 = VGRAMArray(output_dims=(self.cz, 1), pattern_length=cs, min_mem_size=1, max_mem_size=2 ^ 14,
                                     min_dist=2, max_dist=8)
    ...
    def predict(self):
        ...
        # Retrieve hyper-parameters $ \\boldsymbol{\\alpha}_{n|n-1} $ indexed by $ \\check{\bf z}_{n} $ and $ {\\bf b}_{n|n-1} $
        alphas_n_n_1 = self.wnn_layer1.recall(np.concatenate((self.z_n, self.bs_n_n_1)))

        # Draw the predicted posterior $ \\boldsymbol{\pi}_{n+1|n} $ from the Dirichlet distribution $ Dir \left( c_{s}; \\boldsymbol{\\alpha}_{n|n-1} \right) $
        pis_n_1_n = np.ramdom.dirichlet(alphas_n_n_1)

        # Draw up to $ p_{s} $ unique samples from the predicted posterior $ \\boldsymbol{\pi}_{n+1|n} $
        ells = np.unique(np.random.choice(self.cs, size=self.ps, replace=True, p=pis_n_1_n))

        # Build the predicted belief $ {\\bf b}_{n+1|n} $
        self.bs_n_1_n = np.zeros(self.cs, order='C', dtype=bool)
        self.bs_n_1_n[ells] = True

        # Retrieve hyper-parameters $ \\tilde{\\boldsymbol{\\alpha}}_{n|n-1} $ indexed by $ {\\bf b}_{n+1|n} $
        alphaz_n_n_1 = self.wnn_layer2.recall(self.bs_n_1_n)

        # Draw the predicted posterior $ \tilde{\\boldsymbol{\pi}}_{n+1|n} $ from the Dirichlet distribution $ Dir \left( c_{z}; \tilde{\\boldsymbol{\\alpha}}_{n|n-1} \right) $
        piz_n_1_n = np.ramdom.dirichlet(alphaz_n_n_1)

        # Draw up to $ p_{z} $ unique samples from the predicted posterior $ \tilde{\\boldsymbol{\pi}}_{n+1|n} $
        ellz = np.unique(np.random.choice(self.cz, size=self.pz, replace=True, p=piz_n_1_n))

        # Build the predicted belief $ \\tilde{\\bf b}_{n+1|n} $
        self.bz_n_1_n = np.zeros(self.cz, order='C', dtype=bool)
    ...
    def learn(self, z_n_1, k):
        ...
        if self.bz_n_1_n[k] == 1:
            ...
            # Update hyper-parameters $ \\boldsymbol{\\alpha}_{n|n-1} $
            alphas_n_1_n = alphas_n_n_1 + self.bs_n_1_n

            # Retrieve hyper-parameters $ \\tilde{\\boldsymbol{\\alpha}}_{n|n-1} $ indexed by $ {\\bf b}_{n+1|n} $
            alphaz_n_n_1 = self.wnn_layer2.recall(self.bs_n_1_n)

            #  Update hyper-parameters $ \\tilde{\\boldsymbol{\\alpha}}_{n|n-1} $
            alphaz_n_1_n = alphaz_n_n_1 + self.bz_n_1_n
            alphaz_n_1_n[k] += 1
        else:
            # Initialize hyper-parameters $ \\boldsymbol{\\alpha}_{n+1|n} $
            alphas_n_1_n = self.alphas

            # Initialize hyper-parameters $ \\tilde{\\boldsymbol{\\alpha}}_{n+1|n} $
            alphaz_n_1_n = self.alphaz

        # Store new hyper-parameters
    ...
    def encode(self, y_n_1):
        ...
        z_n_1 = np.zeros(self.dz, order='C', dtype=bool)
